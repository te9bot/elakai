...
/* ==========================================================================
 * Picking a photograph for a submission.
 *
 * Chooses a file; it does not upload one. The upload happens when the form is
 * submitted, so abandoning a half-filled form cannot leave an orphaned object
 * in the bucket that nothing references. (The admin picker made the same call
 * for the same reason — see components/admin/image-upload.tsx.)
 *
 * The three states are explicit rather than inferred from a pair of nullable
 * fields, because "no image" and "the existing image was removed" have to be
 * told apart: the first leaves the stored URL alone, the second clears it.
 * ========================================================================== */

export type ImageChoice =
  /** Nothing chosen. On an edit, this means keep whatever is already stored. */
  | { kind: 'none' }
  /** The image already saved against this submission. */
...
export function ImagePicker({
  ...
  const accept = useCallback(
    (file: File | undefined) => {
      ...
      // Advisory only, and asynchronous, so a slow decode never delays the
      // preview appearing.
      void imageQualityWarning(file).then(setWarning)
    },
    [onChange],
  )
    ...
    <div className="min-w-0">
      ...
      {hasImage ? (
        <div className="overflow-hidden rounded-card border border-line bg-canvas">
          {/* `contain` rather than `cover`: this is the contributor checking
              they picked the right file, so it has to show the whole image. A
              crop can hide exactly the part that is wrong. The public card
              crops to its own aspect ratio separately. */}
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-line px-4 py-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-meta text-ink-subtle">
                {value.kind === 'file'
                  ? `${formatBytes(value.file.size)} — uploads when you submit`
                  : 'Already attached to this submission'}
              </p>
            </div>
        ...
        >
          ...
          <p className="mt-1 text-meta text-ink-subtle">
            A clear picture of the front helps people recognise the place.
          </p>
          ...
          <p className="mt-3 text-meta text-ink-subtle">
            JPG, PNG, WEBP or AVIF — up to {formatBytes(MAX_BYTES)}
          </p>
        </div>
      )}
      ...
      {/* A warning, not an error: the image is usable and the submission is
          not blocked. Refusing the only photograph somebody has would be
          worse than publishing a small one. */}
      {warning && !error && (
        <p className="mt-2 flex items-start gap-2 text-meta text-warning-ink">
          <AlertTriangle className="mt-px size-3.5 shrink-0" aria-hidden="true" />
          {warning}
        </p>
      )}
